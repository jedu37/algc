...
//
// A: Yes, this is the correct order to destroy trees, because it destroys the child nodes first and then the parent nodes.
...
void* BSTreeGetMin(const BSTree* header) {
  ...
  // !-. (Done)
  struct _BSTreeNode *currentNode = header->root->left;
...
void* BSTreeSearch(const BSTree* header, const void* item) {
  ...
  while (current != NULL) {
    ...
    // !-. (Done)
    if (header->compare(item, current->item) == 0){
      return current->item;
    }
    if (header->compare(item, current->item) > 0){
...
static void _treeTraverseINOrder(struct _BSTreeNode* root,
  ...
  // Corrija a função para que os nós sejam visitados por ordem crescente. (Done)
  // !-. (Done)
  if (root == NULL) return;
...
// A: Yes, because it allows me to transverse the tree in a non-decreasing manner.
