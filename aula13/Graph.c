...
Graph* GraphCopy(const Graph* g) {
  // COMPLETAR !!
  Graph *copy = GraphCreate(g->numVertices, g->isDigraph, 0);
  for (int i = 1; i < copy->numVertices; i++)
  {
    ListMove(g->verticesList, i);
    ListMove(copy->verticesList, i);
    ListInsert(copy->verticesList, ListGetCurrentItem(g->verticesList));
  }
...
Graph* GraphFromFile(FILE* f) {
  ...
  fscanf(f, "%u\n", &vertexNum);
  fscanf(f, "%u\n", &edgeNum);
  Graph *graph = GraphCreate(vertexNum, isOriented, isWeighted);
  unsigned int v1, v2;
  float w;
  if (!isWeighted)
  ...
  else
  {
    ...
    {
      fscanf(f, "%u %u %f\n", &v1, &v2, &w);
      GraphAddWeightedEdge(graph, v1, v2, (int)w);
    }
  }
